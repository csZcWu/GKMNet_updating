...
class GCM_v2(nn.Module):
    def __init__(self, num_kernels=21, max_kernel_size=21, mode='FG', channels=3):
        ...
        kernel_sizes = [np.min([max_kernel_size, int(init_kernel_size + (i * increment) // 2 * 2)]) for i in
                        range(num_kernels)]
        self.num_k = num_kernels + 1
        self.kernel_paddings = [int((kernel_size + 1) / 2) - 1 for kernel_size in kernel_sizes]
        self.weight = []
        if mode == 'FG':
            for i in range(num_kernels):
                self.weight.append(nn.Parameter(torch.repeat_interleave(
                    gaussian(kernel_sizes[i], 0.25 * (i + 1)).unsqueeze(0).unsqueeze(0).unsqueeze(0), self.channels,
                    dim=0), requires_grad=False).cuda())
        elif mode == 'TG':
            for i in range(num_kernels):
                self.weight.append(nn.Parameter(torch.repeat_interleave(
                    gaussian(kernel_sizes[i], 0.25 * (i + 1)).unsqueeze(0).unsqueeze(0).unsqueeze(0), self.channels,
                    dim=0).cuda(), requires_grad=True))
        else:
            for i in range(num_kernels):
                self.weight.append(nn.Parameter(torch.rand(3, 1, 1, kernel_sizes[i]), requires_grad=True).cuda())

    def __call__(self, x):
        res = [x]
        for i in range(len(self.weight)):
            k_x = self.weight[i]
            k_y = k_x.view((3, 1, 2 * self.kernel_paddings[i] + 1, 1))
            c_x = F.conv2d(F.pad(x, [self.kernel_paddings[i]] * 4, mode='reflect'), k_x, groups=3)
